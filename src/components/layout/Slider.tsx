...
const Slider = ({ desktopOnly, toggleSlider, open, children }: SliderProps) => {
  return (
    <div
      className={`${!open && '-translate-x-[100%]'} ${
        desktopOnly ? 'hidden md:flex' : 'flex '
      } transition ease-in-out duration-300 fixed h-screen md:w-1/2 w-screen bg-white z-30  top-0 left-0 `}
    >
